...
router.get('/', function(req, res){
  var thead=['Name','Type','Size','Color','Unit Price','Status','Action'];
var linkName=['Edit'];
var link=['/_pReport/Edit/product?id='];
var data=[];
  productModel.getAllPaidProducts(function(result){
    for(var i=0; i<result.length; i++){
      data.push([result[i].product_name,result[i].product_type,result[i].product_size,result[i].product_color,result[i].unit_price,result[i].status,result[i].id]);
    }
    console.log(data);
    res.render('report',{
